let nodes = new vis.DataSet([]);
    ...
    let options = {
        edges: {
            ...
            smooth: false // Отключаем сглаживание для более четких стрелок
        }
    };
    ...
    function addEdge() {
        ...
        if (fromNode && toNode && edgeWeight) {
            edges.add({ from: fromNode, to: toNode, label: edgeWeight, value: edgeWeight });
    ...
    function removeSelected() {
        ...
        if (selectedNodes.length > 0) {
            ...
            edges.remove(edges.get().filter(edge => edge.from === selectedNodes[0] || edge.to === selectedNodes[0]));
    ...
    async function startDFS() {
        ...
        async function dfs(nodeId) {
            ...
            const neighbors = edges.get().filter(edge => edge.from === nodeId || edge.to === nodeId);
    ...
    function updateAdjacencyMatrix() {
        ...
        const matrix = Array(nodeArray.length).fill().map(() => Array(nodeArray.length).fill(0));
